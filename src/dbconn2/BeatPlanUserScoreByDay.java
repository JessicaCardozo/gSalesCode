...
public class BeatPlanUserScoreByDay {
	...
	public void updateUserDoc(ViewRow row, HashMap map){
		...
		try{						
			...
			errorLog.writeError(bucket, "customer", "Beat Plan USBD", errorMsg);
		}
	}
	...
	public ViewResult getBeatPlanData(String docName, String viewName) throws Exception{
		...
		try{
			...
			for(ViewRow row : result){
				...
				if(!(sceCode.equals(""))){
					...
					try{				  
						...
						errorMsg = "Decision execution failed for SCE Code "+sceCode;			
						errorLog.writeError(bucket, "customer", "Beat Plan USBD", errorMsg);
						continue;
					}
					...
					try{
						...
						errorMsg = "Record for SCE Code "+sceCode+" could not be updated.";			
						errorLog.writeError(bucket, "customer", "Beat Plan USBD", errorMsg);
						continue;
					}	
				}
			...
			errorMsg = "Error in getBeatPlanData";			
			errorLog.writeError(bucket, "customer", "Beat Plan USBD", errorMsg);
		}					  
