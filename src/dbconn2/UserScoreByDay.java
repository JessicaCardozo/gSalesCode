...
public class UserScoreByDay {
	...
	public void getScoreData(String docName, String viewName) throws Exception{
		...
		for(ViewRow row : result){			
			...
			try{
				...
				errorMsg = "Case Disbursed Score for sceCode "+sceCode+" could not be obtained. "+e.getMessage();			
				errorLog.writeError(bucket, "customer", "User Score By Day", errorMsg);
				continue;
			}
	...
	public void getEnquiryScore(String docName, String viewName) throws Exception{
		...
		for(ViewRow row : result){
			...
			try{
				...
				logger.debug("Enquiry score "+value.toString());
				if(prevSceCode.equals("")){												
					enquiryScore = points;					
				}
				...
				errorMsg = "Enquiry Score for sceCode "+key+" could not be obtained. "+e.getMessage();			
				errorLog.writeError(bucket, "customer", "User Score By Day", errorMsg);
				continue;
			}
			prevSceCode = sceCode;
	...
	/*This method updates enquiry points in the USBD document*/
	...
	/*This method updates all points except enquiry points in the USBD document.*/
	...
	/*This method updates the points to the User document*/
	public void updateUserDoc(String updateViewName, String sceId, HashMap map){
		try{
			...
			errorMsg = "Points for the user with SCE ID "+sceId+" could not be updated.";			
			errorLog.writeError(bucket, "customer", "User Score By Day", errorMsg);
		}
	}
